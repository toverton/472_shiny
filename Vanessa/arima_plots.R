...
auto_corr_lagged <- acf(train_df_ts_lagged, plot = F)
auto_corr_lagged_df <- with(auto_corr_lagged, data.frame(lag, acf))

N <- length(train_df_ts_lagged)
ciz = -qnorm((1-0.95)/2)/sqrt(N-3)
cir = (exp(2*ciz) - 1) / (exp(2*ciz) + 1) 
...
auto_corr_lagged_df |>
  ...
  labs(x = "Lag", 
       ...
       title = "Autocorrelation of Total Fatalities Training Series 
       (After Differencing)") + 
  scale_x_continuous(breaks = round(seq(min(auto_corr_lagged_df$lag), 
                                        max(auto_corr_lagged_df$lag), 
                                        by = 7), 
                                    1))
#ggsave("acf_post_diff.png", width = 12, height = 7)

par_auto_corr_lagged <- pacf(train_df_ts_lagged, plot = F)
par_auto_corr_lagged_df <- with(par_auto_corr_lagged, data.frame(lag, acf))

par_auto_corr_lagged_df |>
  ggplot(mapping = aes(x = lag, y = acf)) +
  geom_hline(aes(yintercept = 0)) +
  geom_segment(mapping = aes(xend = lag, yend = 0)) +
  geom_hline(aes(yintercept = cir), linetype = 2, color = 'firebrick1') +
