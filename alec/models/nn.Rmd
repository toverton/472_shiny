...
```{r}
history_list <- history$metrics
history_df <- as.data.frame(history_list)
...
```{r}
history_long <- pivot_longer(history_df, 
                             cols = c("loss", "val_loss"), 
                             names_to = "Type", 
                             values_to = "Loss")


history_long$Type <- recode(history_long$Type, 
                            loss = "Training", 
                            val_loss = "Validation")
```
...
```{r}
lossplot <-ggplot(history_long, aes(x = epoch, y = Loss, color = Type)) +
  geom_line() +  # Draw lines
  geom_point() +  # Add points for each epoch
...
evaluatetest <- model %>% evaluate(test_X, test_y_scaled)
evaluatetest
evaluatetrain <- model %>% evaluate(train_X, train_y_scaled)
evaluatetrain
```
...
plot2 <- ggplot() +
  ...
  labs(title = "Actual vs. Forecast Total Fatalities Across All Time",
       x = "Date", y = "Total Incidents",
       caption = "Data: Gun Violence Data",
      subtitle = "Long short-term memory (LSTM) Model") +
...
```{r}
plot1 <- ggplot() + geom_line(aes(x = training$date, y = training$total_incidents, 
                         color = "Training" )) + 
  ...
  labs(title = "Actual vs. Forecast Total Fatalities",
       x = "Date", y = "Total Incidents",
       subtitle = "Long short-term memory (LSTM) Model",
       color = "Legend", caption = "Data: Gun Violence Data")+
  theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), 
        axis.line = element_line(colour = "black"),
        legend.title=element_blank()) +
  scale_color_manual(values = c("black", "gray", "#bae4b3")) +
